...
# 5. Start — discovers the mesh automatically from the blockchain
relay-bridge start
```

See the [Bridge Operator Handbook](BRIDGE_OPERATOR_HANDBOOK.md) for the full step-by-step guide.
...
| `seedPeers` | (Optional) Array of `{endpoint, pubkeyHex}` objects — known peers to connect to on startup. Bridges discover peers automatically from the on-chain registry if this is empty. |
...
The backfill queries GorillaPool for unspent outputs at the beacon address (with WhatsOnChain as fallback), then parses each registration/deregistration transaction to build the trusted pubkey whitelist and discover peer endpoints.
